def update_user_sql_not_in_transaction(start, *end):
    if len(end) > 0:
        return "update user u set u.identity = 1 where u.id not in (select distinct user_id from user_role_transaction) and u.id >= %d and u.id < %d;\n" % (start, end[0])
    else:
        return "update user u set u.identity = 1 where u.id not in (select distinct user_id from user_role_transaction) and u.id >= %d;\n" % start

def update_user_sql_in_transaction(start, *end):
    if len(end) > 0:
        return "UPDATE user u JOIN user_role_transaction urt ON urt.user_id = u.id SET u.identity = urt.user_identity WHERE urt.user_id = u.id AND NOT exists(SELECT 1 FROM user_role_transaction urt2 WHERE urt2.user_id = urt.user_id AND urt2.id > urt.id) and u.id >= %d and u.id < %d;\n" % (start, end[0])
    else:
        return "UPDATE user u JOIN user_role_transaction urt ON urt.user_id = u.id SET u.identity = urt.user_identity WHERE urt.user_id = u.id AND NOT exists(SELECT 1 FROM user_role_transaction urt2 WHERE urt2.user_id = urt.user_id AND urt2.id > urt.id) and u.id >= %d;\n" % start
